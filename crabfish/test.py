...
def selfplay():
    ...
    for d in range(200):
        ...
        m, _ = crabfish.search(pos, maxn=200)
        if m is None:
            print("Game over")
            break
...
def testxboard(python='python3'):
    ...
    try:
        ...
        with timeout(5, 'quit did not terminate crabfish'):
            fish.wait()
    finally:
...
def perft(pos, depth, divide=False):
    ...
    for m in pos.genMoves():
        ...
        if not any(pos1.value(m) >= crabfish.MATE_VALUE for m in pos1.genMoves()):
...
def allmate(path):
    with open(path) as f:
        for line in f:
            ...
            _, score = crabfish.search(pos, maxn=1e9)
            if score < crabfish.MATE_VALUE:
    ...
    with open(path) as f:
        for line in f:
            ...
            for d in range(depth, 99):
                s0 = crabfish.bound(pos, 0, d)
                s1 = crabfish.bound(pos, 1, d)
                if s0 >= 0 and s1 < 1:
                    break
...
def quickmate(path, depth):
    ...
    with open(path) as f:
        for line in f:
            ...
            pos = xboard.parseFEN(line)
            for d in range(depth, 99):
                score = crabfish.bound(pos, crabfish.MATE_VALUE, d)
                if score >= crabfish.MATE_VALUE:
                    break
                print(d, score)
            else:
...
def renderSAN(pos, move):
    ...
    i, j = move
    csrc, cdst = crabfish.render(i), crabfish.render(j)
    # Check
    pos1 = pos.move(move)
...
def findbest(path, times):
    ...
    CAL_NODES = 10000
    start = time.time()
    _ = crabfish.search(pos, CAL_NODES)
    ...
    with open(path) as f:
        for k, line in enumerate(f):
            ...
            for t in times:
                move, _ = crabfish.search(pos, factor*t)
