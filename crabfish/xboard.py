...
def parseFEN(fen):
	...
	ep = crabfish.parse(enpas) if enpas != '-' else 0
	score = sum(crabfish.pst[p][i] for i,p in enumerate(board) if p.isupper())
	score -= sum(crabfish.pst[p.upper()][i] for i,p in enumerate(board) if p.islower())
	pos = crabfish.Position(board, score, wc, bc, ep, 0)
	return pos if color == 'w' else pos.rotate()

def mrender(color, pos, m):
	# Crabfish always assumes promotion to queen
	p = 'q' if crabfish.A8 <= m[1] <= crabfish.H8 and pos.board[m[0]] == 'P' else ''
	m = m if color == WHITE else (119-m[0], 119-m[1])
	return crabfish.render(m[0]) + crabfish.render(m[1]) + p

def mparse(color, move):
	m = (crabfish.parse(move[0:2]), crabfish.parse(move[2:4]))
...
def main():
	...
	while True:
		...
		if smove == 'quit':
			...
			m, s = crabfish.search(pos, maxn=nodes)
			# We don't play well once we have detected our death
			if s <= -crabfish.MATE_VALUE:
				print('resign')
			else:
				print('# %d %+d %d %d %s' % (0, s, 0, crabfish.nodes, pv(color,pos)))
